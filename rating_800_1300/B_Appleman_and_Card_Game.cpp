...
class Solution{
     public:
       void sol(){
            ...
            //store the ele with freq
            map<int,int>mp;
            for(int i=0;i<n;i++) mp[cards[i]]++;

            //Now traverse the map, and store in max heap
            priority_queue<int>maxH;
            for(auto it:mp) maxH.push(it.second);

            //now traverse pq, and count maximum 
            ll sum = 0;
            ...
            while(!maxH.empty()){
                ...
                // cout << "top and k & sum "<< top << " " << k << " "<<sum <<endl;
