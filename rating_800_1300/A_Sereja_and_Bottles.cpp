...
class Solution{
     public:
       void sol(){
                ...
                //Now traverse the array, and finding how many bottles can open
        ...
        for(int i=0;i<n;i++){
            ...
            for(int j=0;j<n;j++){
                ...
                //now finding the a, how many bottles can open
                if(arr[i].first == arr[j].second) opened[i] = true;
            }
        
        }

        //now count, no opened bottles
        int count = 0;
        for(int i=0;i<n;i++) if(opened[i] == false) count++;
