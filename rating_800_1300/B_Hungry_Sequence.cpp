...
class Solution{
    bool isPrime(int ele){
        if(ele == 2 || ele == 3 || ele == 5 || ele == 7 || ele == 11 || ele == 13) return true;
        else if(ele%2 == 0 || ele%3 == 0 || ele%5 == 0 || ele%7 == 0 || ele%11 == 0 || ele%13 == 0) return false;

        for(int i=14;i<=sqrt(ele);i++)
            if(ele%i == 0) return false;
        return true;
    }
    int nextPrime(int ele){

        for(int i=ele;i<=1e7;i++){
