...
void solve()
{
   /* int n;
    ...
    //Now covering the b/w caes
    ll odd_range = (n+1)/2;
    ll even_range = n/2;

    // cout << "ranges "<< odd_range << " "<<even_range <<endl;
    ...
    //check where our k move
    if(k <= odd_range){
        //then we traverse from 0 to k and finding the ans value,ans always be odd
       /* int ans = 0,val=1;
        for(int i=0;i<k;i++){
            ans += (val);
            val +=2;
    ...
    //o/w k range is greater
    //then traverse from k-odd_range, and ans will be even only
   /* int ans = 0,val=2;
    for(int i=odd_range;i< k;i++){
        ans += val;
        val +=2;
    }
    print(ans); */
